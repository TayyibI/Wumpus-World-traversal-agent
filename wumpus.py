class WumpusWorld:
  ...
  def print(self):            # print the board state 
    print(self.player)
  ...
  def sim(self, agent):
    ...
    self.has_arrow = True
    self.player = self.initial_location
    while t < 1000:                       # loop 1000 times for upper limit of iterations
      t+=1

      self.print()

      b = self.player in self.breeze       # check if square breezy?
      s = self.player in self.stench       # check if square smelly?
      agent.give_senses(self.player, b, s)  # give senses
